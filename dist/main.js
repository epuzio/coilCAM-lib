...
function $4dbbfebae85fed22$export$2e2bcd8739ae039(amplitude, period, offset, nbPoints, values0, mode = "additive") {
    let values = [];
...
function $bc93fb8b71b9c8d7$export$2e2bcd8739ae039(path0, path1, by_layer = true) {
    let layers = new Set();
    ...
    for (let layer of layers){
        let layer_points0 = path0.filter((p)=>p.z == layer).map((p)=>$bc93fb8b71b9c8d7$var$point([
                p.x,
                p.y
            ]));
        let layer_points1 = path1.filter((p)=>p.z == layer).map((p)=>$bc93fb8b71b9c8d7$var$point([
                p.x,
                p.y
            ]));
...
function $638833b7817d7ae4$export$2e2bcd8739ae039(path0, path1) {
    let layers = new Set();
    path0.sort((a, b)=>a.z - b.z);
    path1.sort((a, b)=>a.z - b.z);
    path0.forEach((point)=>layers.add(point.z));
    ...
    let newPath = [];
    for (let layer of layers){
        let layer_points0 = path0.filter((p)=>p.z == layer).map((p)=>$638833b7817d7ae4$var$point([
                p.x,
                p.y
            ]));
        let layer_points1 = path1.filter((p)=>p.z == layer).map((p)=>$638833b7817d7ae4$var$point([
                p.x,
                p.y
            ]));
...
$parcel$export($ac3d53708de2199d$exports, "getNumTubes", () => $94a0c5bacd65ea53$export$2dad54cf1d4fdb11);
$parcel$export($ac3d53708de2199d$exports, "centerPrint", () => $86cfff78df649917$export$2e2bcd8739ae039);
...
function $f5e5d746e520c5b6$export$c8860df64baac0eb(position, nbPointsInLayer, layerHeight, nozzle_diameter, radius, thickness = 0) {
...
function $f5e5d746e520c5b6$export$a3b482626e4c0b96(position, path, nbPointsInLayer, layerHeight, nozzle_diameter, radius, thickness = 0) {
    // Add lines filling in the base of the vessel
    let basePath = [];
    let height = layerHeight;
    for(let i = 0; i < nbPointsInLayer; i++)basePath.push($f5e5d746e520c5b6$var$point(path[i].x, path[i].y));
...
function $f5e5d746e520c5b6$export$e2253033e6e1df16(position, path, nbPointsInLayer, layerHeight, nozzleDiameter, radius) {
    ...
    let topBase = $f5e5d746e520c5b6$export$c8860df64baac0eb(position, nbPointsInLayer, layerHeight * 2, nozzleDiameter, radius);
...
function $94a0c5bacd65ea53$var$extrude(nozzleDiameter, layerHeight, segmentLen, thicknesses) {
    let points = [];
...
function $94a0c5bacd65ea53$export$1d298176a7b26db(gcode_string, fileName = "COILCAM_GCODE") {
    const blob = new Blob([
        gcode_string
    ], {
        type: "text/plain"
...
function $94a0c5bacd65ea53$export$2dad54cf1d4fdb11(path, nozzleDiameter, layerHeight) {
    let segmentLen = [];
...
window.generateGCode = $94a0c5bacd65ea53$export$6a5b418875592792;
window.getNumTubes = $94a0c5bacd65ea53$export$2dad54cf1d4fdb11;


function $86cfff78df649917$export$2e2bcd8739ae039(path, position, bedDimensions = [
    280,
    265,
    305
]) {
...
window.centerPrint = $86cfff78df649917$export$2e2bcd8739ae039;


function $44565e195065a6f4$var$padSingleParameter(input, inputLength, parameterName) {
...
function $44565e195065a6f4$var$setSingleParameter(input, parameterName, nbLayers, nbPointsInLayer) {
    ...
    if (!input?.length) return $44565e195065a6f4$var$padSingleParameter([], parameterLength, parameterName);
    ...
    else if (input.length < parameterLength) return $44565e195065a6f4$var$padSingleParameter(input, parameterLength, parameterName);
...
function $44565e195065a6f4$var$setParameter(input, parameter_name, nbLayers, nbPointsInLayer) {
    if (parameter_name == "radiusShapingParameter") {
        ...
        if (input?.length && Array.isArray(input[0])) {
            radsp[0] = $44565e195065a6f4$var$setSingleParameter(input[0], parameter_name, nbLayers, nbPointsInLayer);
            radsp[1] = input[1];
        } else {
            radsp[0] = $44565e195065a6f4$var$setSingleParameter(input, parameter_name, nbLayers, nbPointsInLayer);
...
function $93324619b35b7b00$export$2e2bcd8739ae039(path, layerHeight) {
    var nbPointsInLayer = [];
...
window.spiralize = $93324619b35b7b00$export$2e2bcd8739ae039;
